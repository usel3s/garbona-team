...
async function sendBroadcastPayload(telegram, chatId, draft) {
  ...
  const entities = sanitizeEntities(draft.entities || []);
  const text = draft.text || "";

  if (draft.mediaType === "photo" && draft.fileId) {
    return telegram.sendPhoto(chatId, draft.fileId, {
  ...
  if (draft.mediaType === "video" && draft.fileId) {
    return telegram.sendVideo(chatId, draft.fileId, {
      caption: text || undefined,
      caption_entities: entities.length ? entities : undefined,
      reply_markup,
    });
  ...
  if (draft.mediaType === "animation" && draft.fileId) {
    return telegram.sendAnimation(chatId, draft.fileId, {
      caption: text || undefined,
      caption_entities: entities.length ? entities : undefined,
      reply_markup,
    });
