...
const { getVisibleTemplates, isTemplateVisible, addVisibleTemplate } = require("../services/settingsService");
...
async function finishTemplateCreate(ctx, user, code) {
  ...
  try {
    if (id != null) await addVisibleTemplate({ id, name });
  } catch (_) {
  ...
  await upsertBotMessage(
    ...
    [
      ...
      `Доступ: ${flow.isPublic ? "публичный" : "приватный"}`,
      "",
    ...
    if (ctx.session?.linkCreateStep === "ref_create_template_id_input" && ctx.session?.refCreate) {
      ...
      if (!Number.isFinite(templateId) || templateId < 1) {
        await upsertBotMessage(ctx, `${pe("error")} Введите корректный ID шаблона (число).`, {
          reply_markup: referralCreateTemplatesKeyboard(domainId, await getVisibleTemplates()).reply_markup,
        });
        return;
      }
      if (!(await isTemplateVisible(templateId))) {
